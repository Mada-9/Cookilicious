...
const getById = async (req, res, next) => {
  try {
    const cookie = await CookiesModel.findById(req.params.id)
    if(!cookie) {
      return next(next(createError(404, "Cookie not found!")))
    }
    res.status(200).json(cookie)
  } catch (error) {
     next(createError(error.status || 500, error.message, error.details)); 
...
const deleteById = async (req, res, next) => {
  try {
    const cookie = await CookiesModel.findByIdAndDelete(req.params.id)
    if(!cookie) {
      return next(createError(404, "Cookie not found!"))
    }
...
const updateById = async (req, res, next) => {
  try { 
    const cookie = await CookiesModel.findByIdAndUpdate(req.params.id,req.body,{new:true}) //{new:true})= retourne l'element mis à jour et non l'ancien
    if(!cookie) {
      return next(createError(404, "Cookie not found!"))
    }
    res.status(200).json(cookie)
