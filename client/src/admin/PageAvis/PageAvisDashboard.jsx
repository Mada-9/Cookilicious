              ...
              <div key={item._id} className="card mb-3 border-0 shadow-sm rounded-4 overflow-hidden">
                <div className="card-body p-3" style={{color:"var(--marronRouge)"}}>
                  <div className="d-flex justify-content-between align-items-start mb-3">
                    <div className="d-flex align-items-center">
                        <div className="rounded-circle border d-flex align-items-center justify-content-center me-2" style={{width: "35px", height: "35px"}}>
                            <i className="bi bi-person text-secondary"></i>
                        </div>
                        <div>
                            <small style={{fontSize: "11px", color:"var(--marronRouge)"}}>
                              {item.createdAt ? new Date(item.createdAt).toLocaleDateString('fr-FR'): "Date"}
                  ...
                  <p className="small mb-3 italic">
                    "{item.commentaire}"
                  </p>
                  
                  <div className="d-flex justify-content-between border-top pt-3 mt-2">
                    <button onClick={() => toggleStatus(item._id, item.isActive)} className="btn btn-sm flex-grow-1 me-2 border" style={{color:"var(--marronRouge)"}}>
                      <i className="bi bi-power me-1"></i> {item.isActive ? "Masquer" : "Publier"}
                    </button>
                    <button onClick={() => navigate(`/admin/avisdetail/${item._id}`)} className="btn btn-sm flex-grow-1 me-2 border" style={{color:"var(--marronRouge)"}}>
